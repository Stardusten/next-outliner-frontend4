...
export async function handleErrorResponse(
  response: Response,
  error = "Error while handling response"
) {
...
export function filterMessageRole(
  messages: Message[],
  role: MessageRole
): Message[] {
...
export function filterNotMessageRole(
  messages: Message[],
  role: MessageRole
): Message[] {
  return messages.filter((message) => message.role !== role);
...
export function unwrapToolCall(tool_call: WrappedToolCall): ToolCall {
  ...
  if (!tool_call.function.arguments)
    throw new Error("Tool call function arguments is required");
...
export function join(...pathSegments: string[]): string {
  ...
  const isUrl =
    firstSegment.startsWith("http://") || firstSegment.startsWith("https://");
