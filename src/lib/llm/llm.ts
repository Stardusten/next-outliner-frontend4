...
import type { ModelUsageType } from "./model-usage";
...
export default class LLM {
  ...
  constructor(input?: Input | Options, options: Options = {}) {
    ...
    if (typeof options.parser === "string")
      this.parser = this.parsers[options.parser];
  ...
  get isLocal() {
    return (this.constructor as typeof LLM).isLocal;
  }
  get apiKey() {
    if (this.options.apiKey) return this.options.apiKey;
    if (isBrowser()) {
      if (localStorage.getItem(`${this.service.toUpperCase()}_API_KEY`))
        return localStorage.getItem(`${this.service.toUpperCase()}_API_KEY`);
    } else if (isNode()) {
      if (
        // @ts-ignore
        typeof process !== "undefined" &&
        // @ts-ignore
        process.env?.[`${this.service.toUpperCase()}_API_KEY`]
      )
        // @ts-ignore
  ...
  get llmHeaders() {
    ...
    if ((this.constructor as typeof LLM).isBearerAuth)
  ...
  addMessage(role: MessageRole, content: MessageContent) {
    this.messages.push({ role, content });
  }
  user(content: string, attachments?: Attachment[]) {
    if (attachments && attachments.length > 0) {
      const key = (this.constructor as typeof LLM)
        .MessageExtendedContentInputKey;
  ...
  async chat(
    ...
    string | AsyncGenerator<string> | Response | PartialStreamResponse
  > {
  ...
  > {
    ...
    if (vanillaOptions.stream) {
      const body = response.body;
      if (!body) throw new Error("No body found");
      if (this.extended)
        return this.extendedStreamResponse(body, vanillaOptions);
  ...
  protected async *streamResponse(
    ...
    const restream = this.streamResponses(stream, {
      content: this.parseContentChunk.bind(this),
    });
  ...
  protected async *streamResponses(
    stream: ReadableStream,
    parsers: Parsers
  ): AsyncGenerator<Record<string, string | InputOutputTokens | ToolCall[]>> {
    const reader = await parseStream(stream);
    let buffers: Record<
      string,
      string | InputOutputTokens | ToolCall[] | StreamingToolCall
    > = { type: "buffers" };
    ...
    for await (const chunk of reader) {
      for (const [name, parser] of Object.entries(parsers)) {
        ...
        if (name === "usage") {
          ...
          (buffers[name] as ToolCall[]).push(
            ...(content as unknown as ToolCall[])
          );
  ...
  protected async *restream(
    stream: AsyncGenerator<Record<string, string | InputOutputTokens>>,
    callback?: (chunk: Record<string, string | InputOutputTokens>) => void
  ): AsyncGenerator<Record<string, string | InputOutputTokens>> {
    while (true) {
  ...
  protected extendedStreamResponse(
    body: ReadableStream,
    options: Options
  ): PartialStreamResponse {
    let usage: Usage;
    ...
    const restream = this.restream(
      stream as AsyncGenerator<Record<string, string | InputOutputTokens>>,
      (chunk) => {
        if (
          chunk.type === "usage" &&
          chunk.content &&
  ...
  async getModels(quality_filter: QualityFilter = {}): Promise<Model[]> {
    ...
    return models
      .map((model) => {
        let usage =
          ModelUsage.get(this.service, model.model, quality_filter) ||
          ({} as ModelUsageType);

        if (Object.keys(usage).length === 0) {
          // Model not found in ModelUsage database
          ...
          if (this.isLocal) {
            ...
            // For remote models, check allowUnknown first before deciding
            if (!quality_filter.allowUnknown) {
              throw new Error(`model info not found for ${model.model}`);
            }
            // Use default model usage for unknown models when allowed
            usage = this.getDefaultModelUsage(model);
          }
        }
  ...
  parseMessages(messages: Message[]): Message[] {
    return messages.map((message) => {
      ...
      if (copy.role === "thinking" || copy.role === "tool_call")
        copy.role = "assistant";
