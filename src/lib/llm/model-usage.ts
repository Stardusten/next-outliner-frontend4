import { QualityFilter, ServiceName } from "./types";
import data from "./model_prices_and_context_window.json";
...
export default class ModelUsage {
  ...
  static getByServiceModel(
    service: ServiceName,
    model: string
  ): ModelUsageType | null {
    return (
      this.getByService(service).find(
        (m) => m.model === model || m.model === `${service}/${model}`
      ) || null
    );
  }
  ...
  static factories(data: any): ModelUsageType[] {
    ...
    return Object.keys(custom).map((key) => {
      ...
      const output_cost_per_reasoning_token =
        m.output_cost_per_reasoning_token || 0;
      ...
      const supports_function_calling =
        ...
        m.raw?.supports_function_calling ||
        false;
      const supports_vision =
        m.supports_vision || m.raw?.supports_vision || false;
      const supports_web_search =
        m.supports_web_search || m.raw?.supports_web_search || false;
      const supports_audio_input =
        m.supports_audio_input || m.raw?.supports_audio_input || false;
      const supports_audio_output =
        m.supports_audio_output || m.raw?.supports_audio_output || false;
      const supports_prompt_caching =
        m.supports_prompt_caching || m.raw?.supports_prompt_caching || false;
  ...
  static addCustom(info: ModelUsageType) {
    customModels[`${info.service}/${info.model}`] = Object.assign(
      {},
      { mode: "chat" },
      info
    );
  ...
  static removeCustom(service: ServiceName, model: string) {
