import LLM from "./llm";
import type { Model, ServiceName, Options, ToolCall } from "./types";
...
export default class Anthropic extends LLM {
  ...
  parseThinkingChunk(chunk: any): string {
    if (!chunk || chunk.type !== "content_block_delta" || !chunk.delta)
      return "";
  ...
  parseTokenUsage(data: any) {
    if (!data) return null;
    const input_tokens =
      data.message?.usage?.input_tokens || data.usage?.input_tokens;
    const output_tokens =
      data.message?.usage?.output_tokens || data.usage?.output_tokens;
