...
export default class APIv1 extends LLM {
  ...
  parseTools(data: any): ToolCall[] {
    ...
    return data.choices[0].message.tool_calls.map(
      (tool_call: WrappedToolCall) => unwrapToolCall(tool_call)
    );
  }
  ...
  parseToolsChunk(data: any): ToolCall[] {
    if (
      ...
      return [];
    return data.choices[0].delta.tool_calls.map((tool_call: WrappedToolCall) =>
  ...
  parseAttachment(attachment: Attachment) {
    if (attachment.isImage) {
      if (attachment.isURL) {
        return {
          type: "image_url",
          image_url: { url: attachment.data, detail: "high" },
        };
      } else {
        return {
          type: "image_url",
