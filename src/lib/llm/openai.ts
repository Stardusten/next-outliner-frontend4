...
export default class OpenAI extends LLM {
  ...
  parseTokenUsage(data: any) {
    if (data.response && data.type === "response.completed")
      data = data.response;
  ...
  parseContentChunk(chunk: any): string {
    if (!chunk || !chunk.delta || chunk.type !== "response.output_text.delta")
      return "";
    return chunk.delta;
  }
...
function wrapTool(tool: Tool): OpenAITool {
  return {
    name: tool.name,
    parameters: Object.assign({}, tool.input_schema, {
