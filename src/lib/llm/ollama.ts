...
export default class Ollama extends LLM {
  ...
  parseTools(data: any): ToolCall[] {
    ...
    return data.message.tool_calls.map((tool_call: WrappedToolCall) =>
  ...
  parseMessages(messages: Message[]): Message[] {
    ...
    for (const message of messages) {
      ...
      if (message.role === "thinking" || message.role === "tool_call")
      ...
      if (message.role && message.content.attachments) {
        msgs.push({
          role: message.role,
          images: message.content.attachments.map(this.parseAttachment),
        });
        added = true;
      }
