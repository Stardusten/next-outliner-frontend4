...
export const CompletionPopup: Component<Props> = (props) => {
  ...
  const completion = useBlockRefCompletion(props.editor.app);
  ...
  const scrollToActiveItem = () => {
    if (
      ...
      completion.availableBlocks().length === 0
    )
      return;
    const doScroll = () => {
      const activeItem = itemRefs.get(completion.activeIndex());
  ...
  createEffect(() => {
    // react to visibility
    if (completion.visible()) scrollToActiveItem();
  });

  return (
    <Show when={completion.visible()}>
      <div
        ref={(el) => (listRef = el)}
      ...
      >
        <For each={completion.availableBlocks()}>
          {(block, index) => (
            <div
              ...
              onClick={() => completion.handleCompletionSelect(props.editor)}
            >
              <div class="flex-1 overflow-hidden text-foreground">
                <ReadonlyBlockView
                  block={block}
                  app={props.editor.app}
                  searchQuery={completion.query()}
                  showPath
                  // 前十个块不启用延迟渲染，防止闪烁
                  lazyRender={index() >= 10}
                />
        ...
        <Show when={completion.availableBlocks().length === 0}>
